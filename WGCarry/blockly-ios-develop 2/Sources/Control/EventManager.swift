...
@objcMembers public final class EventManager: NSObject {
  ...
  /**
   Fires all pending events.  激活所有挂起的事件
   */
  public func firePendingEvents() {
    ...
    // Work off copy of the current batch of events, so we can clear the queue immediately. 解决当前的一堆事件，我们可以立即清理队列
    let eventQueue = pendingEvents.merged().filterDiscardable()//获得有效的事件队列
