...
@objcMembers open class WorkbenchViewController: UIViewController {
  ...
  open fileprivate(set) lazy var trashCanView: TrashCanView = {
    // Create trash can button 创建垃圾桶按钮
  ...
  */
  ...
  open override func loadView() {
    ...
    // Set default styles
    //工作区的颜色
    workspaceBackgroundColor = .gray
//        ColorPalette.grey.tint50
    ...
    //MARK: - 自注解 toolboxCategoryViewController 是列表延伸出的列表
    toolboxCategoryViewController.view.backgroundColor = .green
//      ColorPalette.grey.tint300.withAlphaComponent(0.75)
    trashCanViewController.view.backgroundColor = .purple
//      ColorPalette.grey.tint300.withAlphaComponent(0.75)
    ...
    // Add child view controllers  添加子控制器
  ...
  open func loadToolbox(_ toolbox: Toolbox) throws {
    // Verify all blocks in the toolbox can be re-created from the block factory 确保名字都在工厂里有效
...
//到这，toolbox已经改变，这步协调者需要同步 反映所有在toolbox里的新的block
...
extension WorkbenchViewController {
  ...
  /**
   Refreshes the UI based on a set of states. 现阶段更新UI
