...
@objcMembers public final class DefaultBlockView: BlockView {
  ...
  /// Flag determining if layer changes should be animated 决定是否动画的鉴定标识
  ...
  */
    ...
    forFlags flags: LayoutFlag = LayoutFlag.All, animated: Bool = false)
  {
    ...
    runAnimatableCode(animated, code: {
      ...
      {
        // Figure out the stroke and fill colors of the block 算出block的边和内部的颜色
...
//  测试用的     strokeColor = .red
//        fillColor = .black
        // Construct the block's bezier path  组装block的曲线
...
//          layout.config.viewUnit(for: DefaultLayoutConfig.BlockLineWidthRegular)
      ...
      {
        // Remove connection highlights (they will be potentially re-added in the completion block). 去除蓝色线条的高亮状态
...
//WARNING:重点来了
  fileprivate func blockBackgroundBezierPath() -> UIBezierPath? {
    ...
    let path = WorkspaceBezierPath(engine: layout.engine) //初始化曲线
    //背景色
...
//    凹凸的宽
    let notchWidth = CGFloat.leastNonzeroMagnitude
//        layout.config.workspaceUnit(for: DefaultLayoutConfig.NotchWidth)
//    凹凸的高
    let notchHeight = CGFloat.leastNonzeroMagnitude
//        layout.config.workspaceUnit(for: DefaultLayoutConfig.NotchHeight)
//    横着的凹凸宽
    let puzzleTabWidth = CGFloat.leastNonzeroMagnitude
//        layout.config.workspaceUnit(for: DefaultLayoutConfig.PuzzleTabWidth)
 //    横着的凹凸高
    let puzzleTabHeight = CGFloat.leastNonzeroMagnitude
//        layout.config.workspaceUnit(for: DefaultLayoutConfig.PuzzleTabHeight)
