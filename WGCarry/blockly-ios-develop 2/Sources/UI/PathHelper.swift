...
@objcMembers public class PathHelper: NSObject {
  ...
  /**
   Adds the path for drawing a next/previous notch. 凹口 目测还有凸起
  ...
  /**
   Adds the path for drawing jagged teeth at the end of collapsed blocks. 锯齿 收缩 暂时没看懂干嘛
  ...
  /**
   Adds the path for drawing a horizontal puzzle tab. 水平的锯齿
...
//    path.moveTo(x: 0, y: blockCornerRadius, relative: true)
  ...
  /**
   Adds the path for drawing a rounded corner. 加圆角
