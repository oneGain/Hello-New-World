...
类代表单独的一个block
 - note: To create a block programmatically, use a `BlockBuilder`.创建一个block ，用BlockBuilder
 */
@objc(BKYBlock)
@objcMembers public final class Block : NSObject {
  ...
  /** 一个元组代表一个连通树
   ...
   - `rootBlock` is the root of the tree    rootBlock 是树根
   - `allBlocks` is a list of all connected blocks (including `rootBlock`).  allBlocks 是列表里所有连接的block
   */
  public typealias BlockTree = (rootBlock: Block, allBlocks: [Block])

  // MARK: - Properties

  /// A unique identifier used to identify this block for its lifetime 唯一标示，用来标明这个block在生命周期里的存在
  public let uuid: String
  /// The type name of this block  名字
  public let name: String
  /// The initial value of `inputsInline`.  `inputsInline`的初始值
  internal let initialInputsInlineValue: Bool
  /// Flag indicating if input connectors should be drawn inside a block (`true`) or
  /// on the edge of the block (`false`)   我猜是 标志表明是否输入连接者应该画入或者包括 block
  ...
  /// The absolute position of the block, in the Workspace coordinate system    绝对位置
  public var position: WorkspacePoint
  /// Flag indicating if this is a shadow block (`true`) or not (`false) 是否带阴影
  ...
  /// List of connections directly attached to this block 附加到block上的连接 的列表
  ...
  /// The color of the block 颜色
  public let color: UIColor
  /// An optional mutator for this block 可选突变
  public var mutator: Mutator?
  /// Tooltip text of the block 工具提示文字
  public var tooltip: String {
    didSet { didSetProperty(tooltip, oldValue) }
  }
  /// The comment text of the block 描述的文字
  public var comment: String {
    didSet { didSetProperty(comment, oldValue) }
  }
  /// A help URL to learn more info about this block  帮助的网址helpURL
  public var helpURL: String {
    didSet { didSetProperty(helpURL, oldValue) }
  }
  /// Flag indicating if this block may be deleted 是否可删除
  public var deletable: Bool {
    didSet { didSetProperty(deletable, oldValue) }
  }
  /// Flag indicating if this block may be moved by the user 是否可以移动
  ...
  /// Flag indicating if this block may be dragged by the user 可以拖拽
  ...
  /// The style that should be applied to the block during rendering. 渲染时的风格
  ...
  /// The layout associated with this block.    此block相关的layout（外部属性，内部一点代码也没有）
    ...
    extensions: [BlockExtension]) throws
  {
    ...
    // Finish updating model hierarchy for inputs and connections 更新模型层
    updateInputs()
    updateDirectConnections()
